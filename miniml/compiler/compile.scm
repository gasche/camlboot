...
(define (mkfield index numfields) (cons index numfields))
(define (get-field-index fdef) (car fdef))
(define (get-field-numfields fdef) (cdr fdef))
...
(define (split-pattern-matching env m)
  (cond ((null? m) (cons (cons #nil #nil) (cons #nil #nil)))
        ...
        ((equal? (car (car (car m))) 'PConstr)
         (let* ((c (car (cdr (car (car m)))))
                (l (car (cdr (cdr (car (car m))))))
                (e (cdr (car m)))
                (t (split-pattern-matching env (cdr m)))
                (consts (car (car t)))
                (blocks (cdr (car t)))
...
(define (compile-switch env stacksize istail e switch)
  (let* ((consts (car (car switch)))
         (blocks (cdr (car switch)))
         (default (car (cdr switch)))
         (nums (cdr (cdr switch))))
    ...
    (if (and (null? consts) (null? blocks))
        ...
        (if (or (null? nums) (< (car nums) 0))
            ...
            (let* ((numconsts (car nums))
              ...
              (for-each (lambda (i)
                          (let* ((p (find (lambda (c) (= i (car (cdr c)))) constslabs))
                                 (lab (if (pair? p) (car p) defaultlab)))
                            (bytecode-emit-labref-with-pos lab basepos))
                          ) (range 0 numconsts))
              (for-each (lambda (i)
                          (let* ((p (find (lambda (b) (= i (car (car (cdr b))))) blockslabs))
                                 (lab (if (pair? p) (car p) defaultlab)))
                            (bytecode-emit-labref-with-pos lab basepos))
                          ) (range 0 numblocks))
              (for-each (lambda (c)
                          (bytecode-emit-label (car c))
                          (compile-expr env stacksize istail (cdr (cdr c)))
                          (bytecode-put-u32-le BRANCH)
                          (bytecode-emit-labref endlab)
                          ) constslabs)
              (for-each (lambda (b)
                          (bytecode-emit-label (car b))
                          (bytecode-put-u32-le PUSH)
                          (compile-bind-fields env (+ stacksize 1) istail (car (cdr (cdr b))) (cdr (cdr (cdr b))))
                          (bytecode-put-u32-le BRANCH)
                          (bytecode-emit-labref endlab)
                          ) blockslabs)
  ...
  (cond ((equal? (car expr) 'EVar)
         ...
         (let* ((name (car (cdr expr)))
           ...
           (if (null? args)
               ...
               (let*
                 ...
                 (compile-args env stacksize nargs)
        ...
        ((equal? (car expr) 'ESetfield)
         (let ((e1 (car (cdr expr)))
           ...
           (bytecode-put-u32-le (get-field-index (env-get-field env f)))))
        ((equal? (car expr) 'ERecord)
         (let* ((l (car (cdr expr)))
                (size (length l))
                (lf (map (lambda (fe)
                           (let* ((fdef (env-get-field env (car fe))))
                             (assert (= (get-field-numfields fdef) size))
                             (cons (get-field-index fdef) (cdr fe)))) l))
                (sf (sort lf (lambda (fe1 fe2) (< (car fe1) (car fe2)))))
                (es (map cdr sf)))
           (for-each (lambda (fe i) (assert (= (car fe) i))) sf (range 0 size))
           (compile-args env stacksize es)
           (bytecode-put-u32-le MAKEBLOCK)
           (bytecode-put-u32-le size)
           (bytecode-put-u32-le 0)
           ))
        ((equal? (car expr) 'ERecordwith)
         (let* ((e (car (cdr expr)))
                 (l (car (cdr (cdr expr))))
                 (size (get-field-numfields (env-get-field env (car (car l)))))
                 (lf (map (lambda (fe)
                            (let* ((fdef (env-get-field env (car fe))))
                              (assert (= (get-field-numfields fdef) size))
                              (cons (get-field-index fdef) (cdr fe)))) l)))
        ...
        ((equal? (car expr) 'EChain)
         (let ((e1 (car (cdr expr)))
           ...
           (compile-expr env stacksize #f e1)
           (compile-expr env stacksize istail e2)))
        ((equal? (car expr) 'EMatch)
         (let* ((e (car (cdr expr)))
                (m (car (cdr (cdr expr))))
                (switch (split-pattern-matching env m)))
           (compile-expr
            env stacksize istail
            (list 'LSwitch e switch))
         ))
        ((equal? (car expr) 'ETry)
         (let* ((e (car (cdr expr)))
                (m (car (cdr (cdr expr)))))
        ...
        ((equal? (car expr) 'LSwitch)
         (let* ((e (car (cdr expr)))
                (switch (car (cdr (cdr expr)))))
           (compile-switch env stacksize istail e switch)
         ))
         ...
         (let* ((bindings (car (cdr expr)))
             ...
             (if (null? bindings)
                 ...
                 (let* ((p (car (car bindings)))
                        ...
                        (rest (cdr bindings))
                        (nlet (list 'ELet rest body)))
                   (expr-fv (list 'EMatch e (list (cons p nlet))) env)))
           ))
        ((equal? (car expr) 'ELetOpen)
         (let* ((m (car (cdr expr)))
                (e (car (cdr (cdr expr))))
        ...
        ((equal? (car tdef) 'IRecord)
         (let* ((l (car (cdr tdef)))
                ...
                (nenv-fields (car (fold (lambda (name ei)
                                          (cons (vhash-replace name (cons #t (mkfield (cdr ei) numfields)) (car ei))
                                                (+ 1 (cdr ei))))
